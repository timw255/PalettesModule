...
using PaletteModule.Web.Controls;
...
namespace PaletteModule
{
	...
	/// </summary>
	public class PaletteModuleClass : ContentModuleBase
	{
		#region Properties
		/// <summary>
		...
		{
			//TODO: add web control automatically
			...
			//var pageNode = new PageManager().GetPageNode(new Guid(pageNodeForPalette.Key));
			//string paletteFromPage = string.Empty;
			//if (pageNode != null)
			//{
			//	if (pageNode.DoesFieldExist("Palette") && pageNode.GetValue("Palette") is string)
			//	{
			//		paletteFromPage = pageNode.GetValue<string>("Palette");
			//	}
			//}

			//string paletteTitle = paletteFromPage.IsNullOrEmpty() ? "Main" : paletteFromPage;

			//DynamicTheme dynamicTheme1 = new DynamicTheme();
			//dynamicTheme1.Palette = paletteTitle;
			//dynamicTheme1.SourceFiles.Add(new ThemeableSourceFile() { Name = "theme.css" });

			//evt.Page.Header.Controls.Add(dynamicTheme1);
		}
