...
namespace PaletteModule.Data.EntityFramework.Decorators
{
    public class PaletteModuleEFDataProviderDecorator : IDataProviderDecorator, IExecutionStateDecorator
    {
        ...
        public void Initialize(string providerName, NameValueCollection config, Type managerType)
        {
            ...
            var provider = this.DataProvider as IPaletteModuleEFDataProvider;

            if (provider != null)
            {
                PaletteModuleEFDataConnection connection = PaletteModuleEFDataConnection.InitializeConnection(this.connectionName, provider);

                provider.ProviderContext = new PaletteModuleEFDataProviderContext()
                {
        ...
        {
            var provider = (IPaletteModuleEFDataProvider)this.DataProvider;
        ...
        {
            var dataProvider = (IPaletteModuleEFDataProvider)this.DataProvider;
        ...
        /// <returns></returns>
        private PaletteModuleEFDbContext GetContext(IPaletteModuleEFDataProvider provider)
        {
            if (provider.ProviderContext == null)
                throw new Exception(String.Format("Provider {0} is not initialized.", provider.Name));

            return PaletteModuleEFDataConnection.GetContext(this.connectionName, provider);
        }
        ...
        /// <returns></returns>
        private string GetScopeId(IPaletteModuleEFDataProvider provider)
        {
